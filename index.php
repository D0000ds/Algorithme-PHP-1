...
// une classe Personne (nom, prénom et date de naissance)
echo "<br>Exercice 15<br>";
...
class Personne 
{
    ...
    public function __construct(string $name, string $surname, string $birthDate)
    {
        ...
        $calculDate = $dateNow - $this->birthDate;

        while ($calculDate > $dateYear){
            $calculYear = $calculYear + 1;
            $calculDate = $calculDate - $dateYear;
        }
        $this->birthDate = $calculYear;

    }

    public function __toString()
    {
        return "$this->name $this->surname a $this->birthDate ans<br>";
    }
